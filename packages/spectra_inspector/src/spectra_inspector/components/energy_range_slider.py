...
def get_element_dropdown_and_slider(
    ...
    among the server's presets (or is None) the first preset is used, and with
    no presets at all the dropdown starts on "none" over the full slider."""
    ...
    if init_element not in element_ranges:
        init_element = elements[0] if elements else "none"
    ...
    slider_init_range = element_ranges.get(init_element, (slider_start, slider_stop))
