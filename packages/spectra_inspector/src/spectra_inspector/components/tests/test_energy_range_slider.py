...
def test_no_presets_renders_a_bare_slider(mocker):
    mocker.patch(_PATCH_TARGET, return_value={})

    cont, ids = ers.get_element_dropdown_and_slider(
        init_element="Mg", slider_start=0.0, slider_stop=15.0
    )
    dropdown, slider = _dropdown_and_slider(cont, ids)

    assert dropdown.options == ["none"]
    assert dropdown.value == "none"
    assert slider.value == (0.0, 15.0)
